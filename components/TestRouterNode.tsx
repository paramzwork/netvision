...
export default function TestRouterNode({
  data,
  selected,
}: NodeProps<RouterNode>) {
  const createHandles = (handles: NodeHandle[] | undefined, side: Position) => {
    ...
    return handles.map((handle, index) => (
      <Handle
        ...
        position={side}
        type={
          side === Position.Left || side === Position.Top ? "target" : "source"
        }
        style={
  ...
  return (
    <div className="@container relative w-full h-full min-h-13 rounded-sm border bg-white">
