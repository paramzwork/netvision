...
import { Activity, Loader2, Network, RefreshCw, Save, Search, Server, Settings, Terminal } from "lucide-react";
...
export default function SystemDevicesSettings() {
  ...
  <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between border-b pb-6">
    <div className="space-y-2">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Manage SNMP settings, discover new network hardware, and configure retention.
        </p>
      </div>
    </div>
    ...
    <div className="flex items-center gap-2">
      <Button variant="outline" size="sm" onClick={() => fetchDevice()} className="h-8 gap-2 hidden lg:flex">
        <Terminal className="h-3.5 w-3.5 text-muted-foreground" />
        Raw Device Fetch
      </Button>
      <Button variant="outline" size="sm" onClick={() => fetchCPU()} className="h-8 gap-2 hidden lg:flex">
    ...
    <div className="p-5 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-end gap-4 rounded-lg bg-muted/30 p-4 border border-dashed">
        <div className="flex flex-col gap-1.5 w-full sm:w-64">
          <Label htmlFor="discover-ip" className="text-xs text-muted-foreground uppercase tracking-wider">Target IP Address</Label>
          <Input
            id="discover-ip"
            type="text"
        ...
        <div className="flex flex-col gap-1.5 w-full sm:w-64">
          <Label htmlFor="discover-comm" className="text-xs text-muted-foreground uppercase tracking-wider">SNMP Community</Label>
          <Input
            id="discover-comm"
            type="text"
            placeholder="public"
      ...
      {/* Results Table */}
      <form onSubmit={handleSubmitDevices} className="rounded-lg border overflow-hidden">
          ...
          <Table>
            <TableHeader className="bg-muted/40">
              <TableRow>
                ...
                <TableHead className="whitespace-nowrap text-right">Current</TableHead>
              ...
              {discoveredDevice.length === 0 ? (
                ...
                discoveredDevice.map((dev, index) => (
                  <TableRow key={index} className="hover:bg-muted/40">
                    <TableCell className="font-medium whitespace-nowrap">{dev.sysName || "—"}</TableCell>
                    <TableCell className="font-mono text-xs whitespace-nowrap">{dev.ipAddress}</TableCell>
                    <TableCell className="whitespace-nowrap"><span className="rounded bg-muted px-2 py-0.5 font-mono text-xs text-muted-foreground">{dev.community}</span></TableCell>
                    <TableCell className="whitespace-nowrap text-sm">{dev.uptime || "—"}</TableCell>
                    <TableCell className="whitespace-nowrap text-sm">{dev.pollTime || "—"}</TableCell>
                    <TableCell className="whitespace-nowrap text-right tabular-nums font-mono text-xs">{dev.currentMs != null ? `${dev.currentMs}ms` : "—"}</TableCell>
                    <TableCell className="max-w-50"><div className="truncate text-sm text-muted-foreground" title={dev.sysDescr}>{dev.sysDescr || "—"}</div></TableCell>
                    <TableCell className="whitespace-nowrap text-sm text-muted-foreground">{dev.sysContact || "—"}</TableCell>
                    <TableCell className="whitespace-nowrap text-sm text-muted-foreground">{dev.sysLocation || "—"}</TableCell>
                    <TableCell className="font-mono text-xs text-muted-foreground whitespace-nowrap">{dev.sysObjectID || "—"}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
  ...
  <section className="rounded-xl border bg-background shadow-sm overflow-hidden">
    ...
    <div className="p-5 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-end gap-4 rounded-lg bg-muted/30 p-4 border border-dashed">
        <div className="flex flex-col gap-1.5 w-full sm:w-64">
          <Label htmlFor="int-ip" className="text-xs text-muted-foreground uppercase tracking-wider">Device IP Address</Label>
          <Input
            id="int-ip"
            type="text"
            placeholder="e.g. 192.168.1.1"
        ...
        <div className="flex flex-col gap-1.5 w-full sm:w-64">
          <Label htmlFor="int-comm" className="text-xs text-muted-foreground uppercase tracking-wider">SNMP Community</Label>
          <Input
            id="int-comm"
            type="text"
            placeholder="public"
      ...
      {/* Results Table */}
      <form onSubmit={handleSubmitInterfaces} className="rounded-lg border overflow-hidden">
              ...
              {interfaces.length === 0 ? (
                ...
                interfaces.map((item) => (
                  <TableRow key={item.index} className="hover:bg-muted/40">
                    <TableCell className="font-mono text-xs tabular-nums text-muted-foreground">{item.index}</TableCell>
                    <TableCell className="font-medium text-sm">{item.name}</TableCell>
                    <TableCell className="max-w-62.5"><div className="truncate text-sm text-muted-foreground" title={item.description}>{item.description || "—"}</div></TableCell>
                    <TableCell className="text-right whitespace-nowrap tabular-nums font-mono text-xs">
                      {item.speedMbps?.toLocaleString() ?? 0} <span className="text-muted-foreground font-sans">Mbps</span>
                    </TableCell>
  ...
  <section className="rounded-xl border bg-background shadow-sm overflow-hidden">
    <div className="flex items-center justify-between border-b bg-muted/20 px-5 py-4">
      ...
      <Button size="sm" variant="outline" onClick={() => fetchDevices()} className="h-8 gap-2">
          ...
          {devices.length === 0 ? (
            ...
            devices.map((dev, index) => (
              <TableRow key={index} className="hover:bg-muted/40 transition-colors">
                ...
                <TableCell className="font-mono text-xs whitespace-nowrap">{dev.ipAddress}</TableCell>
                <TableCell className="whitespace-nowrap"><span className="rounded bg-muted px-2 py-0.5 font-mono text-xs text-muted-foreground">{dev.community}</span></TableCell>
                <TableCell className="whitespace-nowrap text-sm text-muted-foreground">{dev.uptime || "—"}</TableCell>
                <TableCell className="whitespace-nowrap text-sm text-muted-foreground">{dev.pollTime || "—"}</TableCell>
    ...
    <div className="p-6">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 max-w-2xl">
        <div className="space-y-1">
          <h3 className="font-medium text-sm text-foreground">Traffic Data Retention</h3>
          <p className="text-sm text-muted-foreground leading-relaxed">
            Choose how long NetVision keeps interface traffic statistics. Data older than this period will be automatically pruned to save database space.
          </p>
        </div>
